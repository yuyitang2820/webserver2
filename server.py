...
def get_individual():
      ...
      cursor.close()
      data = {'exists':True, 'member_id':result['member_id'], 'firstname':result['firstname'], 'lastname': result['lastname'], 'email': result['email'], 'position': result['position'], 'tshirt_size':result['tshirt_size'], 'phone_number':result['phone_number']}
...
def get_zipcodes():
  ...
  data = []
  for result in cursor:
        info = {'zipcode':result['zipcode'], 'type':result['type'], 'city': result['city'], 'county': result['county'], 'state': result['state'], 'short_state': result['short_state']}
        data.append(info)
  cursor.close()
  context = dict(data = data)
...
def get_majors():
  ...
  data = []
  for result in cursor:
        info = {'major_id':result['major_id'], 'name':result['name'], 'department':result['department'], 'college':result['college']}
        data.append(info)
  cursor.close()
  context = dict(data = data)
...
def add_member():
  ...
  cursor = g.conn.execute(sql_query, (member_id, major_id, zipcode, firstname, lastname, email, position, tshirt_size, phone_number,))
  cursor.close()
...
def get_fees():
  ...
  data = []
  for result in cursor:
        info = {'fee_id':result['fee_id'], 'date':result['date'], 'amount': result['amount'], 'source': result['source'], 'notes': result['notes']}
        data.append(info)
  cursor.close()
  count = {}
  count['size'] = len(data)
...
def add_event():
      ...
      cursor = g.conn.execute(sql_query,(event_id, event_name, event_date, event_type, event_notes, event_location, event_status))
      cursor.close()
...
def get_budget_by_event():
      ...
      if cursor.rowcount == 0:
            ...
            for result in cursor:
                  info = {'budget_id':result['budget_id'], 'category':result['category'], 'spent':result['spent'], 'remaining':result['remaining'], 'amount':result['amount'], 'event_status':result['event_status']}
                  data.append(info)
            cursor.close()
            context = dict(data = data, data2=data2)
            return render_template("budget_by_event.html", **context)
...
def get_events_by_member():
      ...
      if cursor.rowcount == 0:
            ...
            for result in cursor:
                  info = {'event_id':result['event_id'], 'date':result['date'], 'type':result['type'], 'notes':result['notes'], 'location':result['location'], 'status':result['status']}
                  data.append(info)
            cursor.close()
